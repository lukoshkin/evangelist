Review every file you touched this session and fix every quality violation it contains — not just on the lines you changed, and not skipping flaws that predate your changes. Identify those files with `git diff --name-only` (staged and unstaged) plus any untracked files you created, rather than from memory, which drifts after a long session or a context summary. Edits stay confined to that list; reading, though — rule 4 especially — may range across the whole codebase to spot existing abstractions.
...
## 2. Prefer truthiness over `is None` / `is not None` for non-falsy types
...
## 3. No shallow try-except blocks
...
## 4. No duplication; reuse existing abstractions
...
## 5. No in-function or mid-module imports
...
Fixing rule 3 in particular can introduce unbound names or type errors, so do not skip the type check.
...
Then report concisely: the files reviewed; per rule, what was fixed; anything you flagged but deliberately did **not** change (rules 1, 2) with the reason; and the verification result. A silent diff is not an acceptable outcome.
