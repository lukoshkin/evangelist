...
##   row 2: [5h% (↻…) · 7d% (↻…) ·] $cost
##
## zsh variant — macOS compatible (date -r for epoch, no mapfile).

input=$(cat)

fields=( ${(f)"$(echo "$input" | jq -r '
...
five_hr=${fields[5]}
seven_day=${fields[6]}
five_hr_reset=${fields[7]}
seven_day_reset=${fields[8]}

## Short local-time label for a Unix-epoch reset, e.g. "Sat 18:42".
## macOS date uses -r <epoch>, not -d @<epoch>.
fmt_reset() {
  [[ "$1" -gt 0 ]] 2>/dev/null && date -r "$1" +'%a %H:%M' 2>/dev/null
}
