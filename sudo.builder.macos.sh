...
_require_macos() {
  if [[ $(uname) != Darwin ]]; then
    echo 'sudo.builder.macos.sh is intended for macOS only.'
    return 1
  fi
}
...
_ensure_homebrew() {
  ...
  echo 'Homebrew is required to install macOS dependencies.'
...
_install_xcode_cli_tools() {
  ...
  echo 'Xcode Command Line Tools are required for compilers and headers.'
